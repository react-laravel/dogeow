...
export function isBodySettled(body: CANNON.Body, isBall: boolean = false): boolean {
  ...
  const isStable = body.velocity.length() < velocityThreshold && body.angularVelocity.length() < angularThreshold
...
export function checkSceneIsStable(
  ...
  const ballOutOfBounds = ballPos.y < -5 || ballPos.z < -30 || ballPos.z > 15 || Math.abs(ballPos.x) > 12
