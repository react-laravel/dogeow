import type { NavCategory as CategoryData } from '../../types'
...
const makeCategory = (overrides: Partial<CategoryData>): CategoryData => ({
  id: 1,
  name: 'Category',
  icon: null,
  description: null,
...
describe('NavCategory', () => {
  ...
  it('should render category items', () => {
    ...
    render(<NavCategory category={makeCategory({ name: 'Dev', items })} />)
  ...
  it('should return null when items array is empty', () => {
    const { container } = render(
      <NavCategory category={makeCategory({ name: 'Empty', items: [] })} />
    )
  ...
  it('should render multiple categories with items', () => {
    const cat1 = makeCategory({ name: 'Cat1', items: [makeItem({ id: 1 })] })
    const cat2 = makeCategory({ name: 'Cat2', items: [makeItem({ id: 2 })] })
