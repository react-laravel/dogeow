...
export function RepoWatchSsoClient({ returnTo }: { returnTo: string }) {
  ...
  useEffect(() => {
    ...
    void restoreSession()
      .then(user => {
        ...
        setNeedsLogin(true)
        setStatus('登录 DogeOW 后即可进入 Repo Watch')
      })
      .catch(error => {
        setNeedsLogin(true)
  ...
  return (
    <main className="flex min-h-dvh items-center justify-center p-6">
      <section className="bg-card text-card-foreground w-full max-w-md rounded-xl border p-6 shadow-lg">
        <h1 className="text-xl font-semibold">登录 DogeOW Repo Watch</h1>
        <p className="text-muted-foreground mt-2 text-sm">{status}</p>

        {needsLogin ? (
          <form className="mt-6 space-y-4" onSubmit={handleLogin}>
            <div className="space-y-2">
              <Label htmlFor="repo-watch-sso-email">邮箱</Label>
              <Input
                id="repo-watch-sso-email"
                type="email"
                autoComplete="email"
                value={email}
            ...
            <div className="space-y-2">
              <Label htmlFor="repo-watch-sso-password">密码</Label>
              <Input
                id="repo-watch-sso-password"
                type="password"
            ...
            <Button className="w-full" disabled={submitting} type="submit">
              {submitting ? '登录中…' : '登录并进入 Repo Watch'}
            </Button>
          </form>
        ) : (
