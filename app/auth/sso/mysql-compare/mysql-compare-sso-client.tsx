...
export function MysqlCompareSsoClient({ returnTo }: { returnTo: string }) {
  ...
  const issueTicket = useCallback(async () => {
    setStatus('正在安全登录 MySQL Compare…')
  ...
  useEffect(() => {
    ...
    void restoreSession()
      .then(user => {
        ...
        setNeedsLogin(true)
        setStatus('登录管理员账号后即可进入 MySQL Compare')
      })
      .catch(error => {
        setNeedsLogin(true)
  ...
  return (
    <main className="flex min-h-dvh items-center justify-center p-6">
      <section className="bg-card text-card-foreground w-full max-w-md rounded-xl border p-6 shadow-lg">
        <h1 className="text-xl font-semibold">登录 MySQL Compare</h1>
        <p className="text-muted-foreground mt-2 text-sm">{status}</p>

        {needsLogin ? (
          <form className="mt-6 space-y-4" onSubmit={handleLogin}>
            <div className="space-y-2">
              <Label htmlFor="mysql-compare-sso-email">邮箱</Label>
              <Input
                id="mysql-compare-sso-email"
                type="email"
                autoComplete="email"
                value={email}
            ...
            <div className="space-y-2">
              <Label htmlFor="mysql-compare-sso-password">密码</Label>
              <Input
                id="mysql-compare-sso-password"
                type="password"
            ...
            <Button className="w-full" disabled={submitting} type="submit">
              {submitting ? '登录中…' : '登录并进入 MySQL Compare'}
            </Button>
          </form>
        ) : (
