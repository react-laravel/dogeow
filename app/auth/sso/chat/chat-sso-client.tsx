...
export function ChatSsoClient({ returnTo }: { returnTo: string }) {
  ...
  useEffect(() => {
    ...
    void restoreSession()
      .then(user => {
        ...
        setNeedsLogin(true)
        setStatus('登录 DogeOW 后即可进入 Chat')
      })
      .catch(error => {
        setNeedsLogin(true)
  ...
  return (
    <main className="flex min-h-dvh items-center justify-center p-6">
      <section className="bg-card text-card-foreground w-full max-w-md rounded-xl border p-6 shadow-lg">
        <h1 className="text-xl font-semibold">登录 DogeOW Chat</h1>
