...
export const useItem = (id: number) => 
  useSWR<Item>(id ? `/things/items/${id}` : null, fetcher);

// 分类相关 hooks
export const useCategories = () => 
  useSWR('/things/categories', fetcher);

export const useCategory = (id: number) => 
  useSWR(id ? `/things/categories/${id}` : null, fetcher);

// 位置相关 hooks
export const useLocations = () => 
  useSWR('/locations/tree', fetcher);

export const useAreas = <T = unknown>() => 
  useSWR<T>('/areas', fetcher);

export const useArea = (id?: number) => 
  useSWR(id ? `/areas/${id}` : null, fetcher);

export const useRooms = <T = unknown>(areaId?: number) => 
  useSWR<T>(areaId ? `/areas/${areaId}/rooms` : '/rooms', fetcher);

export const useRoom = (id?: number) => 
  useSWR(id ? `/rooms/${id}` : null, fetcher);
...
export const useSpot = (id?: number) => 
  useSWR(id ? `/spots/${id}` : null, fetcher);
