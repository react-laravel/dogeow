...
const DetailsSection: React.FC<DetailsSectionProps> = ({
  ...
  const handleDateChange = useCallback(
    (name: keyof ItemFormData, date: Date | null) => {
    ...
    <Card>
      <CardContent className="space-y-6">
        ...
        <div>
          ...
          <LocationTreeSelect onSelect={onLocationSelect} selectedLocation={selectedLocation} />
          {locationPath && (
            <p className="text-muted-foreground mt-2 text-sm">当前位置: {locationPath}</p>
          )}
        </div>
      </CardContent>
    </Card>
