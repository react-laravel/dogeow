...
import { admin, magicLink, organization, passkey, twoFactor, username } from 'better-auth/plugins'
...
export const publicProcedure = os.$context<{ headers: Headers }>().use(logIdMiddleware)
...
const organizationAdminProcedure = protectedProcedure.use(async ({ context, input, next }) => {
...
export async function getSessionWithCache(
  ...
  if (fresh) {
    // Cache the session
    await redis.set(`${SESSION_CACHE_PREFIX}:${sessionToken}`, JSON.stringify(fresh), {
      ex: SESSION_TTL,
    })
  }
...
export const updateProfile = publicProcedure.handler(async ({ context }) => {
