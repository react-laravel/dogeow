...
const insertedItems = await db.insert(orderItemTable).values(items).returning()
```
 ...
 */
export async function getOrdersWithItems(params: { userId: string; limit?: number }) {
...
const orders = await db.select().from(orderTable).where(ne(orderTable.status, 'DRAFT'))
```
...
const result = await db.transaction(async tx => {
  ...
  const [order] = await tx.insert(orderTable).values({ userId, total: 0 }).returning()
  ...
  await tx.update(orderTable).set({ total }).where(eq(orderTable.id, order.id))
