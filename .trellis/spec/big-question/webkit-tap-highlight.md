...
```tsx
<button className="tap-safe rounded-lg px-4 py-2 bg-blue-500">Click me</button>
```
...
```tsx
<button className="tap-highlight-none rounded-lg px-4 py-2">Click me</button>
```
...
export function TouchSafeWrapper({
  ...
  return (
    <div className={cn(borderRadius, 'overflow-hidden inline-flex', className)}>{children}</div>
  )
}
