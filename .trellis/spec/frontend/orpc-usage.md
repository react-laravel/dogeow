...
export function useItemList(options: UseItemListOptions) {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading, error, refetch } =
    ...
    >({
      ...
      getNextPageParam: (lastPage): ListCursor => lastPage.nextCursor ?? undefined,
...
export function useUpdateItem() {
  ...
  >({
    ...
    onMutate: async ({ itemId, isActive }) => {
      ...
      queryClient.setQueriesData({ queryKey: orpc.items.list.key() }, (old: unknown) => {
        const data = old as {
          pages?: Array<{
        ...
        return {
          ...
          pages: data.pages.map(page => ({
            ...page,
            items: page.items.map(item => (item.id === itemId ? { ...item, isActive } : item)),
          })),
        }
      })
